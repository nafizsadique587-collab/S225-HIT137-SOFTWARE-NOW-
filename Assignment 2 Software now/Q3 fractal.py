...
#!/usr/bin/env python3
"""
Q3: Recursive Fractal (All-in-One)

What it does
------------
- Asks for: number of sides (>=3), side length (float), recursion depth (>=0).
- Generates a Koch-style recursive fractal from a regular polygon.
- Writes outputs:
  * koch_points.txt   — ordered x,y pairs (one per line)
  * koch_summary.txt  — inputs + segments + segment length + approx perimeter + points count
  * koch_plot.png     — (optional) PNG image of the fractal

How to run (interactive prompts)
--------------------------------
python q3_fractal_all_in_one.py

How to run (CLI args)
---------------------
python q3_fractal_all_in_one.py --sides 3 --length 1.0 --depth 3 --outdir .
Add --no-plot to skip PNG even if matplotlib is installed.

Notes
-----
- If matplotlib is not installed, the script still generates the .txt files and
  prints a friendly message about skipping the PNG.
"""
